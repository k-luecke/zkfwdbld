...
| **M4.5 — Scenario synthesis** | **IMPLEMENTED** (pipeline + Control guarantee) / **FRONTIER** (real-protocol deploy graph) | Turns a Seer lead into a full gate scenario (deploy/invariant/control/attack) from a **target-agnostic** template and runs it through the **byte-for-byte real** gate — no fast path. Proven: synthesized **faithful → CONFIRMED**, synthesized **rigged → REJECTED_MALFORMED_CONTROL** (the anti-rigging guarantee: a machine-written rigged predicate is caught by Control). Honest boundary: self-contained run proves the pipeline, not a real finding; signature/permit-scope leads are refused (would misrepresent the bug). Real blind catch needs real-protocol deploy-graph + EIP-712 synthesis — **blind Tier-1 stays 0**. See [docs/M4.5_scenario_synthesis.md](docs/M4.5_scenario_synthesis.md). |
