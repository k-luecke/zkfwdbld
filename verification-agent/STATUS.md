...
| **M4.5 — Scenario synthesis** | **IMPLEMENTED** (pipeline + Control guarantee + deploy-graph discovery) / **FRONTIER** (the semantic mile) | Synthesizes a gate scenario from a lead and runs it through the **byte-for-byte real** gate — no fast path. Proven: **faithful → CONFIRMED**, **rigged → REJECTED_MALFORMED_CONTROL** (anti-rigging guarantee). **Deploy-graph synthesis** derives a target's collaborator deploy+wiring backbone from the M0 model alone; spliced into a hand-built semantic mile it **CONFIRMS Decent M-03 against real code** (calibration) — but the honest ledger shows the backbone is **5/6 autonomous** and a *minority* of a confirming scenario. The real frontier is the **semantic mile** (mock behaviour, payloads, and the core blocker: autonomous **signature** construction for the Control). Centrifuge structural leads were **refuted against real code** (`REJECTED_ATTACK_REVERTED`) — an honest negative result; blind Tier-1 is a **bug-class coverage** gap, **stays 0**. See [docs/M4.5_scenario_synthesis.md](docs/M4.5_scenario_synthesis.md). |
