...
| `model/slither_runner.py` | IMPLEMENTED | Contracts, inheritance, entry points, storage vars, call graph (whole-program: edges for **every** in-scope function — not just entry points — internal callees qualified `Contract.fn`, low-level/delegatecall edged, so downstream reachability follows multi-hop internal flows). Storage *slots* left null (exact packing needs `forge inspect`) — labeled, not guessed. Robust to Foundry projects that need `out/build-info`: on Slither failure it generates build-info and retries with `ignore_compile` (lets the M0 pipeline model fresh contests offline — proven on 2023-11-kelp). |
...
| `pathfind/seer.py` | IMPLEMENTED | Structural-reachability engine; rediscovers the M-03 path from the M0 model. Traverses the call graph **transitively** (qualified internal node = traversable, bare callee = leaf), so a privileged sink reached only via an internal chain is found, not just direct entrypoint callees. |
