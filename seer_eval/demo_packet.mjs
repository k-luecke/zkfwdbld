...
function buildOverview(results = {}, options = {}) {
  ...
  return [
    ...
    '- the same trust-layer product surface from an outbound message action source',
    ...
    `- Message bundle: ${results.messages ? relativeFrom(options.root_dir, results.messages.root_dir) : 'n/a'}`,
    '',
    '## Handoff Snapshot',
    `- Harness readiness: ${harnessSummary.handoff_readiness ?? 'n/a'}`,
    `- Scanner readiness: ${scannerSummary.handoff_readiness ?? 'n/a'}`,
    `- Message readiness: ${messagesSummary.handoff_readiness ?? 'n/a'}`,
    '',
    ...
    '4. Open the message `engineering_handoff.md` to show a non-security action family with `ready_to_send` vs `needs_review` states.',
...
function buildTalkTrack(results = {}, options = {}) {
  ...
  return [
    ...
    'Then open the harness, scanner, and message handoff documents to show that the same trust-layer format works across different input sources and action types.',
    '',
    ...
    `4. Show the message handoff doc and say: ${messagesSummary.handoff_readiness ?? 'Some actions can proceed while others need review.'}`,
    ...
    '- The scanner and message integrations in this packet are mocked, even though the trust-layer surface is real.',
